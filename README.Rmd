...
3. **Matrix of lists** — **columns** are the same time breaks as in (2); each
   ...
   runs** of the same calendar schedule (e.g. posterior draws). Outputs include a
   **`sample`** column indexing the matrix row. With `n_particles > 1`, a
   **`particle`** column indexes stochastic replicates within each run.

To compare **different scenarios as different models** (e.g. no vaccine vs high
vaccine), call **`run_simex()` separately** for each scenario, collect the
`simex` objects in a list, and pass that list to `vis_comparison()`. For
...
**`print()`** summarises: **Time** (day range), **Age categories** (factor
levels in the tables), **Compartments** (`S`–`D`), **Particles** (stochastic
replicates; `1` with the default single particle), and **Samples** (parallel
`pars` rows; `1` when `pars` has a single row). Detailed trajectories are in
`output$prevalence` and `output$incidence` (see **Accessing outputs**).
...
* **`vax`** — logical (`TRUE` / `FALSE`), vaccinated vs unvaccinated strata.
...
Two **different** scenarios require two **`run_simex()`** calls, not a multi-row
`pars` matrix (which is parallel draws for **one** schedule). Pass a **named
list** of `simex` objects to **`vis_comparison()`**.
