...
plot.simex <- function(simex,
  ...
  timeline_tip_dec <- if (use_absolute_numbers) 0L else 2L
  ...
  if (format == "timeline") {
    if (type == "highchart") {
      ...
      hc %>%
        ...
        hc_tooltip(
          ...
          shared = TRUE,
          split = FALSE,
          valueDecimals = timeline_tip_dec,
          headerFormat = paste0(
    ...
    if (type == "highchart") {
      ...
      fit_tooltip_decimals <- if (use_absolute_numbers) 0L else 2L
        ...
        hc_list <- lapply(seq_along(ages), function(i) {
          ...
          hc %>%
            ...
            hc_tooltip(
              ...
              shared = TRUE,
              split = FALSE,
              valueDecimals = fit_tooltip_decimals,
              headerFormat = paste0(
                "<span style=\"font-size:11px\"><b>Day {point.key}</b></span><br/>"
              ),
              pointFormat = paste0(
