...
model with country-specific age demographics and contact rates.

The transmission model is written in the [**odin
DSL**](https://mrc-ide.github.io/odin2/) in
...
| hosp_capacity | Total hospital bed capacity given as number of beds. | 100 |
...
the integer days you want the simulation to run.
...
To visualise the results, use the generic `plot()` method for the
`simex` class. Below we visualise prevalence and then incidence via the
`what` argument.
...
Objects of class `simex` contain **`prevalence`** and **`incidence`**:
...
```
     ...
     1:  age_1           S  FALSE   150  2426
     2:  age_2           S  FALSE   150   494
     3:  age_3           S  FALSE   150   281
     4:  age_4           S  FALSE   150   477
     5:  age_5           S  FALSE   150  1064
     6:  age_6           S  FALSE   150   996
     7:  age_7           S  FALSE   150   950
     8:  age_8           S  FALSE   150   872
     9:  age_9           S  FALSE   150   643
    10: age_10           S  FALSE   150   551
    11: age_11           S  FALSE   150   481
    12: age_12           S  FALSE   150   464
    13: age_13           S  FALSE   150   486
    14: age_14           S  FALSE   150   573
    15: age_15           S  FALSE   150   412
    16: age_16           S  FALSE   150   621
...
```
       ...
       2:  age_2           S  FALSE    10 14011
       3:  age_3           S  FALSE    10 12340
       4:  age_4           S  FALSE    10 10775
...
when `sample` or `particle` are present); for simple shaping, prefer
**`data.table`** as above.
...
1.  **Single list from `get_parameters()`** — the same parameter values
    apply for the whole simulation.

2.  **Named list of parameters** — names must be **numeric strings**
    giving the **first model day** each parameter set applies from.

3.  **Matrix of parameters** — **columns** represent the time breaks as
    in (2). **Rows** are **parallel runs** of the system with different
...
Vaccination is controlled with **`vax_rate`** (doses per day, not a
...
One epidemic with isolation adherence stepping to `0.5` from day 75,
using a **named list** of parameters.
...
Two **different** scenarios require two **`run_simex()`** calls. Pass a
**named list** of `simex` objects to **`plot_simex()`**.
...
plot_simex(outputs, mode = "endpoint", renderer = "ggplot", compartments = "E")
```
